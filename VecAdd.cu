...
bool correct_output(float* a,float* b,float* c, const int n)
{
    for (int i = 0; i < n; i++)
        if (a[i]+b[i]!=c[i])
        {
...
int main(int argc, char *argv[])
{
	...
	checkCudaErrors(cudaMemset(d_b, 255, nbytes));
	checkCudaErrors(cudaMemset(d_c, 255, nbytes));
    ...
    vecAdd<<<blocks, threads, 0, 0>>>(d_a,d_b,d_c);
    ...
    bool bFinalResults = correct_output(a,b,c,n);
	printf("%d\n",bFinalResults);
    // release resources
